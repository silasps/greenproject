...
export default async function PropostaPublicaPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: proposta } = await admin
    .from("propostas")
    .select(
      "km_ida_volta, valor_km, pedagio, alimentacao, valor_servico, custos_extras, valor_total, status, pdf_path, clientes(nome)",
    )
    .eq("token", token)
    .maybeSingle();
    ...
    <div className="mx-auto flex max-w-md flex-col items-center px-4 py-20 text-center sm:px-6">
      ...
      {pdfUrl && (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-6 rounded-md border border-neutral-300 px-6 py-3 font-semibold text-neutral-700 hover:bg-neutral-50"
        >
